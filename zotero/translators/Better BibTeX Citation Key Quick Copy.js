{
	...
	"inRepository": false,
	"configOptions": {
		"hash": "093f65bbd563e2f91d5c4834c474005f4940e3cc627a31e85ec3cd2b032cefca"
	},
...
var { citeCreators, doExport, yearFromDate } = (() => {
  ...
  function resolvePath(templatePath, options) {
    ...
    if (baseFilePath) {
      ...
      } else resolvedFilePath = join(dirname(baseFilePath), templatePath);
    } else resolvedFilePath = join(views, templatePath);
...
`;
  ...
  var Eta$1 = class {
    ...
    loadTemplate(name2, template, options) {
      if (typeof template === "string") (options?.async ? this.templatesAsync : this.templatesSync).define(name2, this.compile(template, options));
      else {
        let templates = this.templatesSync;
        if (template.constructor.name === "AsyncFunction" || options?.async) templates = this.templatesAsync;
        templates.define(name2, template);
      }
    }
  };
